...
import { Download, Settings, Map as MapIcon, Layout, RefreshCcw, MapPin, Type as TypeIcon, Palette, Compass, List, Loader2, CheckCircle2, Bug } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (city.id === 'url-target') return; // 避免初始加载循环
    const params = new URLSearchParams(window.location.search);
  ...
  }, [city.lat, city.lng, zoom, selectedStyle, city.accentColor, city.name, isDebug]);
  ...
  return (
    <div className={`min-h-screen bg-[#050505] text-white flex flex-col md:flex-row overflow-hidden ${isExporting ? 'exporting-active' : ''}`}>
      ...
      <main className={`flex-1 relative bg-[radial-gradient(circle_at_center,_#111_0%,_#050505_100%)] ${isBatchMode ? 'overflow-y-auto custom-scrollbar' : 'flex items-center justify-center p-12'}`}>
        {!isBatchMode ? (
          <div ref={previewRef} data-testid="wallpaper-canvas" className="relative w-full max-w-[340px] aspect-[9/19] animate-float">
          ...
          <div className="w-full py-20 flex flex-col items-center gap-20">
            {PRESET_CITIES.map(c => (
              <div key={c.id} className="w-[340px] aspect-[9/19] relative">
